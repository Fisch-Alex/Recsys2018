...
from Generate_features_scenario8 import Generate_features_scenario8 as Generate_features
...
class LGB_Model8:
	...
	def __init__(self,parameters,num_tracks_considered,playlists):
		...
		self.vectorizers=[tv11]

	def fit(self,playlist_info,playlists,track_info_dict,num_boost_round,early_stopping_rounds,number_of_samples,FPs,num_seed_track = 100,verbose_eval=1,tuning=True,test_size=0.2,seed=134):
		...
		parameters = [seed, int(number_of_samples/cores), FPs, num_seed_track, len(self.list_to_try), False]
		...
		X,self.vectorizers = Generate_features(Subsampled_data_frame,playlist_info,playlists,track_info_dict,self.list_to_try,self.CV_list_of_common_tracks,self.CV_list_of_common_albums,self.CV_list_of_common_artists,
			self.vectorizers,True)
