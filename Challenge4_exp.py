...
NAME = "name_first_five"

from lgb_model4_exp import LGB_Model4 as model
from Load_Playlists import Load_Playlists 
from Load_Testdata import Load_Playlist_file, Load_Meta
from Generate_features_scenario4 import Generate_features_scenario4 as Generate_features
...
if (DEBUG):
	...
	playlist_info = pd.read_csv("data/playlists_info.csv",nrows = 1000000,encoding = "latin-1")
...
ourmodel.fit(playlist_info,playlists,track_info_dict,num_boost_round=rounds,early_stopping_rounds=300,number_of_samples=N,FPs=4)
...
print('[{}] Begin Predicting'.format(time.time() - start_time))
p = Pool(10)
...
p.close()
print('[{}] Finished Predicting.'.format(time.time() - start_time))

ourpredictions = ourpredictions1 + ourpredictions2 + ourpredictions3 + ourpredictions4

print("hi5")

with open("Submissions_exp/challenge4_RAW_NEW_LARGE.csv", "w") as f:
	writer = csv.writer(f)
