...
class LGB_Model4:
	...
	def __init__(self,parameters,num_tracks_considered,playlists):
		...
		self.vectorizers=[tv1,tv2,tv3,tv4,tv5,tv6,tv7,tv8,tv9,tv10]

	def fit(self,playlist_info,playlists,track_info_dict,num_boost_round,early_stopping_rounds,number_of_samples,FPs,num_seed_track = 5,verbose_eval=1,tuning=True,test_size=0.2,seed=134):
		...
		print('[{}] Album and artists added.'.format(time.time() - start_time))
		...
		print('[{}] Henry Sampler.'.format(time.time() - start_time))
		...
		print('[{}] Features.'.format(time.time() - start_time))
