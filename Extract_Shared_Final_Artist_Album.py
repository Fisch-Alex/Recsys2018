...
def num_last(x,y):
	return(max([loc for loc, val in enumerate(x) if val != y]+[-1]))
...
def Extract_Shared_Final_Artist_Album(featuredaf,Subsampled_data_frame,track_info_dict):
	...
	featuredaf["ARTIST_NUMBER_LAST_IN_PLAYLIST"] = np.array(list(map(num_last,Artists_in_seed,[track_info_dict[ids]['artist_id'] for ids in Subsampled_data_frame["TO_PREDICT"]])))
	featuredaf["ALBUM_NUMBER_LAST_IN_PLAYLIST"]  = np.array(list(map(num_last,Albums_in_seed, [track_info_dict[ids]['album_id']  for ids in Subsampled_data_frame["TO_PREDICT"]])))
