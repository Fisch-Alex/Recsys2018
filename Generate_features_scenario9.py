...
from Extract_Shared_Artist_Album import Extract_Shared_Artist_Album
...
def Generate_features_scenario9(Subsampled_data_frame,playlist_info,playlists,track_info_dict,list_to_try,CV_list_of_common_tracks,CV_list_of_common_albums,CV_list_of_common_artists,list_of_vectorisers,Training):
	...
	Extract_Shared_Artist_Album(featuredaf,Subsampled_data_frame,track_info_dict)
	...
	if Training:
		Huge_BOW, vectorizers = BOW(Subsampled_data_frame,playlist_info,playlists,track_info_dict,Training,list_of_vectorisers,9)
		CHAR_GRAM  = tv11.fit_transform(playlist_names)
		vectorizers.append(tv11)
	else:
		Huge_BOW = BOW(Subsampled_data_frame,playlist_info,playlists,track_info_dict,Training,list_of_vectorisers,9)
		CHAR_GRAM = tv11.transform(playlist_names)

	output = hstack((featuredaf,CHAR_GRAM,SPARSE_ARTIST_ALBUM_TRACK_ARRAY,Huge_BOW)).tocsr()
