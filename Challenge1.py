...
from lgb_model1 import LGB_Model1 as model
...
print("hi3")
test_playlists     = [[] for ii in range(1000)]
...
def Get_Features(ii): 
	...
	tobetested = [x for x in ourmodel.list_to_try if x not in playlist]
	...
	X_test  = Generate_features(testdata,info,playlists,track_info_dict,ourmodel.vectorizers,False)
...
print("hi5")
with open("Submissions_exp/challenge1_RAW_NEW_LARGE.csv", "w") as f:
	writer = csv.writer(f)
